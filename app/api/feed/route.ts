...
export async function GET() {
  ...
  if (!user) return NextResponse.json({ error: 'Not authenticated', items: [] }, { status: 401 })
  ...
  if (followingIds.length === 0) {
    return NextResponse.json({ items: [], empty: true, message: 'Follow people to see their activity here.' })
  }
  ...
  const [{ data: checkins }, { data: saves }] = await Promise.all([
    ...
    admin.from('reactions')
      .select('user_id, venue_sanity_id, venue_type, venue_slug, created_at, profiles(username, display_name, avatar_url)')
      .in('user_id', followingIds)
  ...
  return NextResponse.json({ items: items.slice(0, 30), empty: false })
}
